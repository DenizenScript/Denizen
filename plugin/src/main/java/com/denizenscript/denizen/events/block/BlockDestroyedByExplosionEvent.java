...
public class BlockDestroyedByExplosionEvent extends BukkitScriptEvent implements Listener {
    ...
    public boolean matches(ScriptPath path) {
        ...
        if (path.switches.containsKey("source_entity") && (entityEvent == null || !new EntityTag(entityEvent.getEntity()).tryAdvancedMatcher(path.switches.get("source_entity"), getTagContext(path)))) {
            return false;
        }
        if (path.switches.containsKey("source_block") && (blockEvent == null || !new LocationTag(blockEvent.getBlock().getLocation()).tryAdvancedMatcher(path.switches.get("source_block"), getTagContext(path)))) {
            return false;
        }
