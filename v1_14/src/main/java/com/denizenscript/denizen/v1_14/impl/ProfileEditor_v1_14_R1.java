...
import com.denizenscript.denizen.v1_14.helpers.PacketHelper_v1_14_R1;
import com.denizenscript.denizen.v1_14.impl.packets.handlers.DenizenNetworkManager_v1_14_R1;
...
public class ProfileEditor_v1_14_R1 extends ProfileEditor {
    ...
    protected void updatePlayer(Player player, final boolean isSkinChanging) {
        ...
        for (Player p : Bukkit.getServer().getOnlinePlayers()) {
            if (!p.getUniqueId().equals(uuid)) {
                PacketHelper_v1_14_R1.sendPacket(p, destroyPacket);
            }
        }
        new BukkitRunnable() {
            @Override
            public void run() {
                ...
                for (Player player : Bukkit.getServer().getOnlinePlayers()) {
                    PacketHelper_v1_14_R1.sendPacket(player, playerInfo);
                    if (!player.getUniqueId().equals(uuid)) {
                        PacketHelper_v1_14_R1.sendPacket(player, spawnPacket);
                    }
                    else {
                        if (isSkinChanging) {
    ...
    public static boolean handleMirrorProfiles(PacketPlayOutPlayerInfo packet, DenizenNetworkManager_v1_14_R1 manager,
