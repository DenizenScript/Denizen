...
public class ConstantTags implements Listener {
    ...
    public void constantTags(ReplaceableTagEvent event) {
        ...
        if (npc.hasTrait(ConstantsTrait.class)
        		&& event.getNPC().getCitizen().getTrait(ConstantsTrait.class).getConstant(event.getValue()) != null)
        	event.setReplaceable(event.getNPC().getCitizen().getTrait(ConstantsTrait.class).getConstant(event.getValue()));
        }
