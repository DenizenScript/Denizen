...
public class FlagManager {
	...
	public FlagManager(Denizen plugin) {
		this.plugin = plugin;
		this.dB = plugin.getDebugger();
	}
	...
	public class Flag {
		...
		public void setEntireValue(Object obj) {
			plugin.getSaves().set(flagPath, obj);
			plugin.saveSaves();
			rebuild();
			return;
		}
		 ...
		 */
		public void save() {
			plugin.getSaves().set(flagPath, value.values);
			plugin.getSaves().set(flagPath + "-expiration", expiration);
			plugin.saveSaves();
		}
		...
		private void checkExpired() {
			rebuild();
			if (plugin.getSaves().contains(flagPath + "-expiration")) 
				if (expiration > 1 && expiration < System.currentTimeMillis()) {
					plugin.getSaves().set(flagPath + "-expiration", null);
					plugin.getSaves().set(flagPath, null);
					rebuild();
		...
		private Flag rebuild() {
			if (plugin.getSaves().contains(flagPath + "-expiration"))
				this.expiration = (plugin.getSaves().getLong(flagPath + "-expiration")); 
			List<String> cval = plugin.getSaves().getStringList(flagPath);
			if (cval == null) {
				cval = new ArrayList<String>();
				cval.add(plugin.getSaves().getString(flagPath, ""));
			}
