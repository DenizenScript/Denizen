...
public class ItemHelper_v1_9_R2 implements ItemHelper {
    ...
    public PlayerProfile getSkullSkin(ItemStack is) {
        net.minecraft.server.v1_9_R2.ItemStack itemStack = CraftItemStack.asNMSCopy(is);
        if (itemStack.hasTag()) {
    ...
    public CompoundTag getNbtData(ItemStack itemStack) {
        ...
        if (nmsItemStack != null && nmsItemStack.hasTag()) {
            return CompoundTag_v1_9_R2.fromNMSTag(nmsItemStack.getTag());
        }
        return new CompoundTag_v1_9_R2(new HashMap<String, Tag>());
    }

    @Override
    public ItemStack setNbtData(ItemStack itemStack, CompoundTag compoundTag) {
        net.minecraft.server.v1_9_R2.ItemStack nmsItemStack = CraftItemStack.asNMSCopy(itemStack);
        nmsItemStack.setTag(((CompoundTag_v1_9_R2) compoundTag).toNMSTag());
        return CraftItemStack.asBukkitCopy(nmsItemStack);
    }
    ...
    public ItemStack setAttributeModifiers(ItemStack itemStack, Map<EntityAttribute, List<EntityAttributeModifier>> modifiers) {
        ...
        for (Map.Entry<EntityAttribute, List<EntityAttributeModifier>> entry : modifiers.entrySet()) {
            ...
            for (EntityAttributeModifier modifier : entry.getValue()) {
                ...
                modifierList.add(new CompoundTag_v1_9_R2(compound));
            }
        }
